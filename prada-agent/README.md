...
|---|-----|--------|-----------------|
| 1 | `prada setup` missing module | ✅ Fixed | Created `prada_cli/setup.py` with interactive wizard |
| 2 | `prada start` import error | ✅ Fixed | Fixed import path in `prada_cli/callbacks.py` to use dynamic sys.path |
| 3 | AIAgent constructor mismatch | ✅ Fixed | Removed invalid `prada_home` and `callback` parameters |
| 4 | ContextEngine abstract class instantiation | ✅ Fixed | Use `get_context_engine()` factory function instead of direct instantiation |
| 5 | DefaultContextEngine None config | ✅ Fixed | Added null-safe config handling: `(config or {}).get()` |
